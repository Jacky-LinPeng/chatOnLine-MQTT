    ...
    if (self) {
        ...
        */
        ...
        //隐藏菜单
        {
            ...
            textView.frame = CGRectMake(5, 7, toolW - 10 - 54, 35);
            textView.layer.masksToBounds = YES;
            textView.layer.cornerRadius = 35 / 2.0;
            textView.layer.borderWidth = 0.5;
            ...
            UIImage *keyboardImg = [UIImage imageNamed:@"send-s-5"];
            WZMInputBtn *btn = [WZMInputBtn chatButtonWithType:WZMInputBtnTypeSend];
            btn.frame = CGRectMake(toolW-54, 5, 44, 44);
            [btn setImage:keyboardImg forState:UIControlStateNormal];
            [btn setImage:keyboardImg forState:UIControlStateSelected];
            [btn addTarget:self action:@selector(btnClick:) forControlEvents:UIControlEventTouchUpInside];
            [self addSubview:btn];
            [array addObject:btn];
...
- (void)btnClick:(UIButton *)btn {
    //linpeng
    if ([self.delegate respondsToSelector:@selector(toolView:didSend:)]) {
        [self.delegate toolView:self didSend:WZMInputBtnTypeSend];
    }
    return;
