...
@interface WZMChatViewController ()<UITableViewDelegate,UITableViewDataSource,UIGestureRecognizerDelegate,WZMInputViewDelegate,XPYMQTTManagerProxy>
...
- (instancetype)initWithSession:(WZMChatSessionModel *)sessionModel {
    ...
    if (self) {
        ...
        //linpeng
        self.showName = YES;
        self.sessionId = sessionModel.sid;
        self.title = sessionModel.name;
        [[XPYMQTTManager sharedInstance] addObserver:self];
        [[XPYMQTTManager sharedInstance] addSubscribeWithTopic:self.sessionId];
...
- (void)inputView:(WZMInputView *)inputView sendMessage:(NSString *)message {
    ...
    WZMChatUserModel *user = [WZMChatUserModel shareInfo];
    user.uid = deviceID;
    user.name = [NSString stringWithFormat:@"%@(%@)",[[XPYMQTTManager sharedInstance] getDeviceName],[deviceID substringToIndex:4]];
    user.showName = YES;
    
    //MQTT同步
    [[XPYMQTTManager sharedInstance] sendTxtMessage:@{@"user":[user yy_modelToJSONObject], @"msg":message} topicId:self.sessionId];
    
    //清空草稿
    [[WZMChatDBManager DBManager] removeDraftWithModel:user];
     WZMChatMessageModel *model = [WZMChatMessageManager createTextMessage:user
                                                                message:message
                                                               isSender:YES];
    [self sendMessageModel:model];
}
...
-(void)didReciveMessage:(NSDictionary *)data topicId:(NSString *)topicId {
    if (![topicId isEqualToString:self.sessionId]) {
        return;
    }
    
    NSDictionary *userInfo = data[@"user"];
    NSString *txt = data[@"msg"];
    WZMChatUserModel *user = [WZMChatUserModel yy_modelWithJSON:userInfo];
    
    //自己
    NSString *deviceID = [UIDevice currentDevice].identifierForVendor.UUIDString;
    if ([deviceID isEqualToString:user.uid]) {
        return;
    }
